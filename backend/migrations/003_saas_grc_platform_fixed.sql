-- ===============================================
-- MULTI-TIER SAAS GRC PLATFORM MIGRATION (SQLITE COMPATIBLE)
-- ===============================================
...
-- Add new columns to existing tenant_users table (without defaults)
...
ALTER TABLE tenant_users ADD COLUMN is_active BOOLEAN;
...
ALTER TABLE frameworks ADD COLUMN created_by_admin_id TEXT;
ALTER TABLE frameworks ADD COLUMN updated_by_admin_id TEXT;
ALTER TABLE frameworks ADD COLUMN version TEXT;
ALTER TABLE frameworks ADD COLUMN change_log TEXT;
ALTER TABLE frameworks ADD COLUMN is_premium BOOLEAN;
ALTER TABLE frameworks ADD COLUMN sort_order INTEGER;

-- Update existing frameworks with defaults
UPDATE frameworks SET version = '1.0' WHERE version IS NULL;
UPDATE frameworks SET is_premium = false WHERE is_premium IS NULL;
UPDATE frameworks SET sort_order = 0 WHERE sort_order IS NULL;
...
CREATE TABLE tenant_control_implementations (
  ...
  customized_fields TEXT DEFAULT '[]', -- JSON as TEXT for compatibility
...
ALTER TABLE tenant_frameworks ADD COLUMN subscription_date DATETIME;
ALTER TABLE tenant_frameworks ADD COLUMN auto_update BOOLEAN;
...
-- Update existing tenant_frameworks with defaults
UPDATE tenant_frameworks SET subscription_date = datetime('now') WHERE subscription_date IS NULL;
UPDATE tenant_frameworks SET auto_update = true WHERE auto_update IS NULL;
